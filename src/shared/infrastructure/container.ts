...
import { SampleViewerProvider } from "@/modules/github-insights/infrastructure/sample-viewer.provider";
 ...
 * else has to.
 *
 * It is rebuilt per request rather than shared, because once GitHub sign-in
 * lands (ticket 03) the adapters are constructed with the signed-in viewer's
 * credentials. The stores it wires stay process-wide, so what you watch
...
const viewerProvider = new SampleViewerProvider();
